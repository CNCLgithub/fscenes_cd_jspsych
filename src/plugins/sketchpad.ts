...
// https://github.com/jspsych/jsPsych/blob/main/packages/plugin-sketchpad/src/index.ts
//
// To ensure that the `Finish` button is not visable until something is drawn
...
class SketchpadPlugin implements JsPsychPlugin<Info> {
  ...
  private init_display() {
    ...
    if (this.params.show_finished_button) {
      finish_button_html = `<p id="finish-btn"><button class="jspsych-btn" id="sketchpad-end" disabled>${this.params.finished_button_label}</button></p>`;
    }
    ...
    let display_html;
    display_html = timer_html + canvas_html + `<div class="sketchpad_response">` + sketchpad_controls + this.params.prompt + finish_button_html + `</div>`;

    this.display.innerHTML = `<div >`  + display_html + "</div>";
  ...
  private start_draw(e) {
    const btn = this.display.querySelector("#sketchpad-end");
    btn.disabled = false;
